...
TEST_CASE("Parsing with the mock")
{
    ...
    auto lldp = std::make_shared<lldp::lldp::LLDPDataProvider>(std::filesystem::path(CMAKE_CURRENT_SOURCE_DIR "/tests/files/"s) / dataDir, std::move(sdbus::createSessionBusConnection()), serverBus);
