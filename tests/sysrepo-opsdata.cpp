...
TEST_CASE("Sysrepo opsdata callback")
{
    ...
    const std::string bus = "local2.org.freedesktop.network1",
                      object = "/local2/org/freedesktop/network1",
                      interface = "local2.org.freedesktop.network1.Manager";
    ...
    auto lldp = std::make_shared<lldp::lldp::LLDPDataProvider>(CMAKE_CURRENT_SOURCE_DIR "/tests/files/single-link", *dbusClientConnection, bus, object, interface);
