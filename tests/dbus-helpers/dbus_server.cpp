    ...
    : m_manager(sdbus::createObject(connection, objectPath))
{
    ...
    m_manager->registerMethod("ListLinks").onInterface(interface).implementedAs([this]() { return ListLinks(); });
...
std::vector<sdbus::Struct<int, std::string, sdbus::ObjectPath>> DbusServer::ListLinks() const
{
    return m_links;
}