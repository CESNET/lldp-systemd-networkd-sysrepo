...
void sd_lldp_neighbor_deleter::operator()(sd_lldp_neighbor* e) const {
    sd_lldp_neighbor_unrefp(&e);
}

LLDPDataProvider::LLDPDataProvider(std::filesystem::path dataDirectory, sdbus::IConnection& dbusConnection)
    : m_dataDirectory(std::move(dataDirectory))
    , m_networkdDbusProxy(sdbus::createProxy(dbusConnection, "org.freedesktop.network1", "/org/freedesktop/network1"))
{
}
...
std::vector<std::pair<int, std::string>> LLDPDataProvider::listLinks() const
{
    ...
    m_networkdDbusProxy->callMethod("ListLinks").onInterface("org.freedesktop.network1.Manager").storeResultsTo(links);
 ...
 */
std::unique_ptr<sd_lldp_neighbor, sd_lldp_neighbor_deleter> LLDPDataProvider::nextNeighbor(std::ifstream& ifs)
{
    size_t rawSz; // get neighbour size in bytes

    // read neighbor size; also (by systemd): each LLDP packet is at most MTU size, but let's allow up to 4KiB just in case
...
std::vector<NeighborEntry> LLDPDataProvider::getNeighbors() const
{
    ...
    for (const auto& [linkId, linkName] : listLinks()) {
        ...
        if (!ifs.is_open()) {
            spdlog::debug("  failed to open LLDP datafile ({}). LLDP is probably not enabled for this link.", lldpFilename);
            continue;
        }

        for (sd_lldp_neighbor_managed n = nextNeighbor(ifs); n != nullptr; n = nextNeighbor(ifs)) {
            NeighborEntry ne;
